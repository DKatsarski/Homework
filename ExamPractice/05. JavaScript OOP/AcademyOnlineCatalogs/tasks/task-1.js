function solve() {
	...
	class Catalog {
		...
		// mojebi towa se slaga, ako trqbwa da ne slagame nishto, pri polojenie, che ima gershka
		add(...items) {
		...
		find(x) {
			...
			if (x !== null && typeof x === 'object') {
				return this.items.filter(function (item) { // kak da se naredi masiwa
					return Object.keys(x).every(function (prop) { // kliuchowete na nastoqshtiq obekt
						return x[prop] === item[prop]; //ako propertitata sywapadat.
					});
				});
			}
		...
		search(pattern) {
			...
			return this.items.filter(function (item) {
				return item.name.indexOf(pattern) >= 0 // ne go chatkam
	...
	return {
		getBook: function (name, isbn, genre, description) {
			new Book(description, name, isbn, genre);
		},
		getMedia: function (name, rating, duration, description) {
			new Media(description, name, duration, raiting);
		},
		getBookCatalog: function (name) {
			new BookCatalog(description, name, isbn, genre);
		},
		getMediaCatalog: function (name) {
			new MediaCatalog(description, name, isbn, genre);
		}
	};
}
